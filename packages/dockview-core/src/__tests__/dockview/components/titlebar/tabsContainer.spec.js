...
const tabsContainer_1 = require("../../../../dockview/components/titlebar/tabsContainer");
const dom_1 = require("@testing-library/dom");
const dockviewGroupPanelModel_spec_1 = require("../../dockviewGroupPanelModel.spec");
...
describe('tabsContainer', () => {
    test('that an external event does not render a drop target and calls through to the group mode', () => {
        ...
        const cut = new tabsContainer_1.TabsContainer(accessor, groupPanel);
        ...
        jest.spyOn(emptySpace, 'offsetHeight', 'get').mockImplementation(() => 100);
        jest.spyOn(emptySpace, 'offsetWidth', 'get').mockImplementation(() => 100);
        dom_1.fireEvent.dragEnter(emptySpace);
        dom_1.fireEvent.dragOver(emptySpace);
    ...
    test('that a drag over event from another tab should render a drop target', () => {
        ...
        dataTransfer_1.LocalSelectionTransfer.getInstance().setData([
            new dataTransfer_1.PanelTransfer('testcomponentid', 'anothergroupid', 'anotherpanelid'),
    ...
    test('that dropping over the empty space should render a drop target', () => {
        ...
        const groupPanel = new groupPanelMock();
        const cut = new tabsContainer_1.TabsContainer(accessor, groupPanel);
        cut.openPanel(new dockviewGroupPanelModel_spec_1.TestPanel('panel1', jest.fn()));
        cut.openPanel(new dockviewGroupPanelModel_spec_1.TestPanel('panel2', jest.fn()));
        const emptySpace = cut.element
            .getElementsByClassName('dv-void-container')
            .item(0);
        if (!emptySpace) {
    ...
    test('left actions', () => {
        ...
        expect(query.length).toBe(1);
        expect((_a = query[0].children.item(0)) === null || _a === void 0 ? void 0 : _a.className).toBe('test-left-actions-element');
        ...
        query = cut.element.querySelectorAll('.dv-tabs-and-actions-container > .dv-left-actions-container');
        expect(query.length).toBe(1);
        expect((_b = query[0].children.item(0)) === null || _b === void 0 ? void 0 : _b.className).toBe('test-left-actions-element-2');
        expect(query[0].children.length).toBe(1);
        // remove left action
    ...
    test('right actions', () => {
        ...
        expect(query.length).toBe(1);
        expect((_a = query[0].children.item(0)) === null || _a === void 0 ? void 0 : _a.className).toBe('test-right-actions-element');
        expect(query[0].children.length).toBe(1);
        // add right action
        ...
        query = cut.element.querySelectorAll('.dv-tabs-and-actions-container > .dv-right-actions-container');
        expect(query.length).toBe(1);
        expect((_b = query[0].children.item(0)) === null || _b === void 0 ? void 0 : _b.className).toBe('test-right-actions-element-2');
        expect(query[0].children.length).toBe(1);
        // remove right action
    ...
    test('that a tab will become floating when clicked if not floating and shift is selected', () => {
        ...
        (0, dom_1.fireEvent)(container, event2);
        expect(accessor.addFloatingGroup).toHaveBeenCalledTimes(1);
    ...
    test('that selecting a tab with shift down will move that tab into a new floating group', () => {
        ...
        const groupPanel = new groupPanelMock();
        const cut = new tabsContainer_1.TabsContainer(accessor, groupPanel);
        const createPanel = (id) => (0, shoehorn_1.fromPartial)({
            id,
            view: {
                tab: {
                    element: document.createElement('div'),
    ...
    test('class dv-single-tab is present when only one tab exists`', () => {
        const cut = new tabsContainer_1.TabsContainer((0, shoehorn_1.fromPartial)({
            options: {},
            onDidOptionsChange: jest.fn(),
        }), (0, shoehorn_1.fromPartial)({}));
        expect(cut.element.classList.contains('dv-single-tab')).toBeFalsy();
        const panel1 = new dockviewGroupPanelModel_spec_1.TestPanel('panel_1', (0, shoehorn_1.fromPartial)({}));
        cut.openPanel(panel1);
        expect(cut.element.classList.contains('dv-single-tab')).toBeTruthy();
        const panel2 = new dockviewGroupPanelModel_spec_1.TestPanel('panel_2', (0, shoehorn_1.fromPartial)({}));
        cut.openPanel(panel2);
        expect(cut.element.classList.contains('dv-single-tab')).toBeFalsy();
    ...
    describe('updateDragAndDropState', () => {
        test('that updateDragAndDropState calls updateDragAndDropState on tabs and voidContainer', () => {
            ...
            const groupPanel = (0, shoehorn_1.fromPartial)({
                id: 'testgroupid',
                model: (0, shoehorn_1.fromPartial)({}),
            });
            const cut = new tabsContainer_1.TabsContainer(accessor, groupPanel);
            ...
            cut.tabs = mockTabs;
            cut.voidContainer = mockVoidContainer;
    ...
    describe('tab overflow dropdown with close buttons', () => {
        test('close button should be visible and clickable in dropdown tabs', () => {
            ...
            const mockPanel = (0, shoehorn_1.fromPartial)({
                id: 'test-panel',
                api: {
                    isActive: false,
                    close: mockClose,
            ...
            cut.toggleDropdown({ tabs: ['test-panel'], reset: false });
            ...
            dom_1.fireEvent.click(dropdownTrigger);
            ...
            dom_1.fireEvent.click(closeButton);
        ...
        test('clicking tab content (not close button) should activate tab', () => {
            ...
            dom_1.fireEvent.click(tabContent);
