...
const splitviewComponent_1 = require("../../splitview/splitviewComponent");
const splitviewPanel_1 = require("../../splitview/splitviewPanel");
class TestPanel extends splitviewPanel_1.SplitviewPanel {
    getComponent() {
        return {
            update: () => {
                //
...
describe('componentSplitview', () => {
    ...
    test('that the container is not removed when grid is disposed', () => {
        ...
        const splitview = new splitviewComponent_1.SplitviewComponent(container, {
    ...
    test('api', () => {
        ...
        panel1 === null || panel1 === void 0 ? void 0 : panel1.api.setActive();
        expect(panel1.api.isActive).toBeTruthy();
        expect(panel2.api.isActive).toBeFalsy();
    });
    test('vertical panels', () => {
        const disposables = new lifecycle_1.CompositeDisposable();
        const splitview = new splitviewComponent_1.SplitviewComponent(container, {
            orientation: splitview_1.Orientation.VERTICAL,
            createComponent: (options) => {
                switch (options.name) {
        ...
        const panel1 = splitview.getPanel('panel1');
        const panel2 = splitview.getPanel('panel2');
        let panel1Dimensions;
        disposables.addDisposables(panel1.api.onDidDimensionsChange((event) => {
            panel1Dimensions = event;
        }));
        let panel2Dimensions;
        disposables.addDisposables(panel2.api.onDidDimensionsChange((event) => {
    ...
    test('serialization', () => {
        ...
        expect(container.querySelectorAll('.dv-split-view-container').length).toBe(1);
        splitview.fromJSON({
            views: [
                {
                    size: 1,
